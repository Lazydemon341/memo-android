...
internal class ImagesScoringModelAdapter @Inject constructor(
    ...
    private fun processImage(imageMetaData: ImageMetaData): ScoredImage {

        if (!isInited) {
            return ScoredImage(imageMetaData, 0f)
        }

        val bitmap = if (VERSION.SDK_INT >= VERSION_CODES.P) {
            getImageByUri(imageMetaData.uri)
        } else {
            return ScoredImage(imageMetaData, 0f)
        }
        ...
        return ScoredImage(imageMetaData, results?.size?.toFloat() ?: 0f)
    }
    ...
    private companion object {
        ...
        private const val MODEL_FILENAME = "scoring_model.tflite"
